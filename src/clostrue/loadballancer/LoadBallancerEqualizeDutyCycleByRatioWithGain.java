 ...
 */
public class LoadBallancerEqualizeDutyCycleByRatioWithGain extends LoadBallancerEmpty implements LoadBallancer{
   
    private static boolean LOG_TO_CONSOLE = false;
    private static boolean WITH_GAIN = true;
    
    public LoadBallancerEqualizeDutyCycleByRatioWithGain(Simulation inSimulation) {
        super(inSimulation, "Equalize DutyCycles By Ratio with Gain",LOG_TO_CONSOLE);
    }
...
// do the load ballancing
    ...
    public void ballanceWork() {
        ...
        ArrayList<TaskMeasure> measures = prepareMeasuresBasedOnDutyCycle(WITH_GAIN);
        ...
        if (measures.size() > 1){
            ...
            Double destinationsRelativeValuesSum = calculateMeasureRelativeValueSumForDestinationTasks(measures);
            calculateDestinationBallancingFactorsByRelation(measures, destinationsRelativeValuesSum);
